...
class SearchController extends Controller
{
    public function index(Request $request) {
        ...
        // $articulos = Article::where('titulo','like',"%$busqueda%")->ArticulosActivos()->with(['imagenes'])->orderBy('id','desc')->get();
        ...
        if(auth()->check()) {
            ...
            if(!is_null(auth()->user()->email_verified_at)) {
                if(!auth()->user()->bloqueado) {
                    $articulos = Article::where('titulo','like',"%$busqueda%")->with(['imagenes'])->orderBy('id','desc')->get();
                    return view('buscador')->with(compact('articulos'));
                }
